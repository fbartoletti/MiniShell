...
void add_env_node(t_environment **env, t_environment *new)
{
    ...
    {
        *env = new;
        new->prev = new;  // Riferimento circolare per il primo nodo
        return;
    }
    ...
    last->next = new;
    new->prev = last;
    (*env)->prev = new;  // Aggiorna il prev del primo nodo per puntare all'ultimo nodo
}
...
void init_environment(t_terminal *term, char **env)
{
    ...
    // Inizializza la lista collegata dell'ambiente
    term->env = NULL;
    i = 0;
    while (env[i])
    {
